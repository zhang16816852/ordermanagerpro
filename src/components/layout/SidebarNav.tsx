...
export function SidebarNav({ navItems, collapsed: navCollapsed, onNavClick }: SidebarNavProps) {
  const { user, isAdmin, isRep, isFixEngineer, storeRoles, signOut } = useAuth();
                ...
                {!navCollapsed && (
                  <div className="flex-1 text-left min-w-0">
                    ...
                    <p className="text-xs text-sidebar-foreground/60">
                      {isAdmin ? '系統管理員' : isRep ? '業務' : isFixEngineer ? '維修人員' : storeRoles?.[0]?.role === 'founder' ? '創辦人' : storeRoles?.[0]?.role === 'manager' ? '經理' : storeRoles?.[0]?.role === 'employee' ? '員工' : 'Customer'}
                    </p>
                  </div>
                )}
              </Button>
