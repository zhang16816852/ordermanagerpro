...
import { ArrowLeft, Search, ChevronRight, ChevronDown, Wallet, Percent, FileText, Store, CheckCircle2, Banknote, Undo2, CalendarClock } from 'lucide-react';
...
export default function RepCommissionPage() {
  ...
  const { registerPayout, revokePayout, bulkRegisterPayout } = useCommissionPayout();

  // 業務資料
  const { data: repRole } = useQuery({
    queryKey: ['admin-rep-detail', userId],
    queryFn: async () => {
  ...
  const { data: repProfile } = useQuery({
    queryKey: ['admin-rep-profile-detail', userId],
    queryFn: async () => {
      const { data, error } = await (supabase
  ...
  const { data: repCosts = [] } = useQuery<{ product_id: string; variant_id: string | null; cost: number }[]>({
    queryKey: ['admin-rep-costs-detail', userId],
    queryFn: async () => {
      const { data, error } = await (supabase
  ...
  const { data: repAssignments = [] } = useQuery<{ store_id: string }[]>({
    queryKey: ['admin-rep-assignments-detail', userId],
    queryFn: async () => {
      const { data, error } = await (supabase
  ...
  const { data: salesNotes = [], isLoading: notesLoading } = useQuery<SalesNoteRecord[]>({
    queryKey: ['admin-rep-commission-notes', userId, assignedStoreIds.join(',')],
    queryFn: async () => {
        ...
        .select(`
          ...
          sales_note_items(
            ...
            order_item:order_items(
              product_id, variant_id, unit_price, unit_cost,
  ...
  // 登記收款時間（連動會計模組 accounting_entries）
  const { data: paymentEntries = [] } = useQuery<PaymentEntry[]>({
    queryKey: ['admin-rep-commission-payments', userId, salesNoteIds.join(',')],
    queryFn: async () => {
  ...
  const { data: payouts = [] } = useQuery<PayoutRecord[]>({
    queryKey: ['admin-rep-commission-payouts', userId],
    queryFn: async () => {
      const { data, error } = await (supabase
        .from('rep_commission_payouts') as any)
        .select('id, rep_id, sales_note_id, paid_date, amount')
        .eq('rep_id', userId!)
  ...
  // 會計帳戶 / 分類（供發放 Dialog 與批次發放使用）
  const { data: accounts = [] } = useQuery<Account[]>({
    queryKey: ['admin-rep-commission-accounts'],
    queryFn: async () => {
  ...
  const { data: categories = [] } = useQuery<AccountingCategory[]>({
    queryKey: ['admin-rep-commission-categories'],
    queryFn: async () => {
      const { data, error } = await (supabase as any)
  ...
  const payoutCategoryId = useMemo(() => {
    return categories.find(c => c.type === 'expense' && c.name.includes('分潤'))?.id
  ...
  const noteStats = useMemo(() => {
    return filteredNotes.map(sn => {
      ...
      for (const item of sn.sales_note_items || []) {
        ...
        if (!oi) continue;
        const snapshotCost = Number((oi as any).unit_cost) || 0;
        const cost = snapshotCost > 0 ? snapshotCost : getCost(oi.product_id, oi.variant_id ?? null);
  ...
  const payoutTargetSn = payoutTargetId
    ? noteStats.find(sn => sn.id === payoutTargetId) || null
    : null;

  const payableNoteIds = useMemo(
    () => noteStats.filter(n => n.totalCommission > 0 && !n.payout).map(n => n.id),
    [noteStats],
  );
  ...
  return (
    <div className="space-y-6 pb-10">
      <PageHeader
        title="業務佣金明細"
        subtitle={`${repProfile?.full_name || '—'}（${repProfile?.email || ''}）`}
        icon={<Wallet className="h-5 w-5 text-amber-500" />}
        actions={
          <Button variant="ghost" size="sm" onClick={() => navigate('/admin/reps')}>
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <CardTitle>銷貨單明細</CardTitle>
            <div className="flex items-center gap-2 flex-wrap">
              <Button
                variant="outline"
                size="sm"
                onClick={() => setBulkDialogOpen(true)}
                disabled={summary.payableCount === 0}
              >
                <Banknote className="h-4 w-4 mr-1" />
                批次登記發放（{summary.payableCount} 筆）
              </Button>
            </div>
          </div>
                    ...
                    return (
                      <Fragment key={sn.id}>
                            ...
                            {sn.payout ? (
                              <div className="flex items-center gap-2">
                                <Badge variant="outline" className="border-emerald-500 text-emerald-600">
                                  已發放 {new Date(sn.payout.paid_date).toLocaleDateString('zh-TW')}
                                </Badge>
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className="h-6 w-6"
                                  title="撤銷發放（回衝帳戶）"
                                  onClick={(e) => { e.stopPropagation(); revokePayout.mutate(sn.payout!.id); }}
                                >
                              ...
                              <Button
                                ...
                                size="sm"
                                className="h-7"
                                disabled={sn.totalCommission <= 0}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setPayoutTargetId(sn.id);
                                  setPayoutDialogOpen(true);
                                }}
                              >
                                <Banknote className="h-3.5 w-3.5 mr-1" /> 登記發放
                              </Button>
                            )}
                          </TableCell>
                        ...
                        {isExpanded && sn.sales_note_items?.map((item, idx) => {
                          const oi = item.order_item;
                          if (!oi) return null;
                          const snapshotCost = Number(oi.unit_cost) || 0;
                          const cost = snapshotCost > 0 ? snapshotCost : getCost(oi.product_id, oi.variant_id ?? null);
      ...
      {/* 登記發放 Dialog（統一使用 EntryDialog 的「佣金發放」Tab） */}
      <EntryDialog
        open={payoutDialogOpen}
        onOpenChange={(o) => { setPayoutDialogOpen(o); if (!o) setPayoutTargetId(null); }}
        categories={categories}
        accounts={accounts}
        isLoading={registerPayout.isPending}
        prefill={
          payoutTargetSn
            ? {
                payout: { repId: userId, salesNoteId: payoutTargetSn.id },
                transactionDate: format(new Date(), 'yyyy-MM-dd'),
                description: `分潤發放 ${payoutTargetSn.code}`,
                categoryId: payoutCategoryId || undefined,
              }
            : null
        }
        onSubmit={() => {}}
        onPayoutSubmit={(payload) => registerPayout.mutate(payload, {
          onSuccess: () => {
      ...
      {/* 批次登記發放 Dialog（複選 → 統一 EntryDialog「佣金發放」Tab，金額由後端重算） */}
      <EntryDialog
        open={bulkDialogOpen}
        onOpenChange={(o) => setBulkDialogOpen(o)}
        categories={categories}
        accounts={accounts}
        isLoading={bulkRegisterPayout.isPending}
        prefill={{
          payout: {
            repId: userId,
            salesNoteIds: payableNoteIds,
          },
          transactionDate: format(new Date(), 'yyyy-MM-dd'),
          description: `業務分潤批次發放 ${repProfile?.full_name || ''}`,
          categoryId: payoutCategoryId || undefined,
        }}
        onSubmit={() => {}}
        onBatchPayoutSubmit={(payload) => bulkRegisterPayout.mutate(payload, {
