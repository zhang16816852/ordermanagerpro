...
### 其他
- 共享連結用 `access_token`（UUID）+ `get_shared_order_details`/`get_shared_sales_note_details`/`get_shared_consignment_details` RPC；路由 `/share/order/:orderId`、`/share/sale/:salesNoteId`、`/share/consignment/:consignmentId`
