-- ============================================================
-- Add p_warehouse_id parameter to receiving/shipping RPCs
-- Allows callers to specify which warehouse to deduct from
-- instead of always using 'own'. Falls back to 'own' if NULL.
-- ============================================================

-- ============================================================
-- 1. receive_purchase_items
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    VALUES (v_product_id, v_variant_id, v_warehouse_id, v_received_qty, 'purchase_receipt', v_po_id, v_po_code);
  END LOOP;
END;
$$;
...
BEGIN
  ...
  FOR v_store_id IN SELECT unnest(p_store_ids) LOOP
    ...
    VALUES (v_store_id, p_created_by, 'shipped', v_shipped_at, p_notes, v_access_token)
...
BEGIN
  ...
  INSERT INTO public.sales_notes (store_id, created_by, status, shipped_at, notes, access_token)
  VALUES (v_order.store_id, p_created_by, 'shipped', v_shipped_at, v_sn_notes, v_access_token)
...
BEGIN
  ...
  INSERT INTO public.sales_notes (store_id, created_by, status, shipped_at, notes, access_token)
  VALUES (p_store_id, p_created_by, 'shipped', v_shipped_at, p_notes, v_access_token)
  RETURNING id, code INTO v_sales_note_id, v_sales_note_code;
