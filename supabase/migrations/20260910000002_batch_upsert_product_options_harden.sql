-- ============================================================
-- 20260910000002_batch_upsert_product_options_harden.sql
-- 加固 batch_upsert_product_options：在每個 jsonb_array_elements 前
-- 以 jsonb_typeof 檢查是否為合法陣列，非陣列就視為空陣列，避免
-- 前端誤送 string/scalar 時拋「cannot extract elements from a scalar」。
-- 簽名不變（CREATE OR REPLACE），前端型別不需重產。
-- ============================================================
