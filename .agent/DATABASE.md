...
> Schema 權威來源為 `supabase/migrations/`（70 支 SQL）；`src/integrations/supabase/types.ts` 為自動產生。
...
| 表 | 說明 | 關鍵欄位 |
|---|---|---|
| `consignment_orders` | 寄賣單（雙方向統一） | code(`CS-YYMMDD-XXXXX`)、direction(`receive_from_supplier`/`send_to_store`)、supplier_id/store_id（依方向擇一，CHECK 強制）、status(`draft`/`active`/`settled`/`cancelled`)、source_order_id（訂單轉寄賣時回填 orders.id） |
| `consignment_order_items` | 寄賣品項 | consignment_order_id、product_id、variant_id、quantity、unit_price（建議售價/出貨價）、unit_cost（進貨成本） |
...
| RPC | 功能 |
|---|---|
| `ship_from_pool(p_store_ids, p_created_by, p_notes, p_warehouse_id, p_warehouse_map, p_source_map)` | 從出貨池批次出貨：依門市建 sales_note(status=shipped) → 產生 sales_note_items → 更新 order_items.shipped_quantity/status → 扣庫存 → 清空該門市 pool → 更新 order.status；`p_source_map` 逐項指定庫存來源（self 扣 own 倉 / supplier_consignment 走 allocate_inventory）。**寄賣單（order.consignment_mode=true）品項只寫 sales_note_items(inventory_source_type='store_consignment')，不扣自有庫存**，單店送完後呼叫 `create_consignment_shipment_layer` 自動建寄賣單 |
| `direct_ship_order(p_order_id, p_created_by, p_notes, p_warehouse_id, p_warehouse_map, p_source_map)` | 訂單直接轉銷貨：建 sales_note → 為剩餘數量建 items → 更新 order_items → 扣庫存 → order 標 shipped；同上支援來源選擇。**整單依 v_order.consignment_mode 分流**（寄賣模式不扣庫存、結尾呼叫 layer） |
| `create_order_with_sales_note(p_store_id, p_created_by, p_items JSONB, p_notes, p_shipped_at, p_warehouse_id, p_consignment_mode)` | 下單即出貨：建 order(source_type=admin_proxy, status=shipped) + sales_note + items + 扣庫存；`p_items[]` 逐項可帶 `inventory_source_type`。`p_consignment_mode=true` 時訂單標寄賣、items 一律 store_consignment、跳過 own 扣庫存、結尾呼叫 layer |
| `create_consignment_shipment_layer(p_sales_note_id, p_warehouse_id, p_created_by)` | 訂單轉寄賣中間層（v1.1 新增）：依 sales_note_items→order_items→orders 找 `consignment_mode=true` 品項，find-or-create `consignment_order`(send_to_store, source_order_id, draft/active) + `consignment_order_items` + `consignment_out_shipment` movement（owner=store_consignment，sign 為負）；被 ship_from_pool / direct_ship_order / create_order_with_sales_note 呼叫 |
...
| `create_consignment_shipment(p_consignment_order_id, p_created_by, p_notes, p_shipped_at)` | 店家方向出貨：依**尚未出貨數量**（`order_quantity - shipped_quantity`）建 order(source_type='consignment', status=shipped) + sales_note + order_items + sales_note_items(inventory_source_type='store_consignment') + `consignment_out_shipment` movement（own 倉）；回傳 order/sales_note 資訊。⚠️ 出貨量**不能用** summary.`remaining_quantity`（那是「店家手上未售」＝shipped−sold−returned，未出貨時為 0） |
...
| `report_consignment_sale(p_consignment_order_item_id, p_quantity, p_sale_price, p_note, p_created_by)` | 店家回報銷售：檢查方向=send_to_store + 可回報數量（扣除 pending 回報）→ insert `consignment_sales_reports`(pending)；回傳 report_id |
| `confirm_consignment_sales(p_report_ids UUID[], p_confirmed_by)` | 後台審核店家回報：逐筆驗證 pending + 剩餘量 → insert `consignment_sales`(store_report) + 標 confirmed |
