...
| `sales_note_items` | 銷貨單明細 | sales_note_id、order_item_id（FK NOT NULL，錨定來源訂單品項）、quantity、returned_quantity、inventory_source_type（self/supplier_consignment/store_consignment，逐項記錄庫存來源）、sort_order（品項顯示順序） |
...
| `purchase_orders` | 採購單 | supplier_id、status(`purchase_order_status`)、order_date、expected_date、received_date、total_amount、**purpose**（`general`/`repair_parts`，2026-09-12：維修叫料標記，`RepairPurchaseDialog` 建立時寫 `'repair_parts'`，前端列表類型篩選用） |
...
| 表 | 說明 | 關鍵欄位 |
|---|---|---|
| `consignment_orders` | 寄賣單（雙方向統一） | code（草稿 `CS-DRAFT-{id8}`，出貨 `CS{YYMM}{店碼}{0001}`）、direction(`receive_from_supplier`/`send_to_store`)、supplier_id/store_id（依方向擇一，CHECK 強制）、status(`draft`/`active`/`settled`/`cancelled`)、source_order_id（訂單轉寄賣時回填 orders.id；**v1.4 起 send_to_store 一建立即同步建來源 order 並回填**）、received_at/received_by（店家確認收貨，v1.3）、shipped_at（出貨時間，寄賣碼月份依據）；send_to_store 非 draft/cancelled 強制 source_order_id（CHECK `chk_consignment_send_to_store_source_order`） |
...
- `sales_notes`／`consignment_orders`：分享驗證為 **OR 條件**——`access_token = p_token::UUID` **或** `share_token_for_code(code) = p_token::UUID`（`get_shared_sales_note_details`／`get_shared_consignment_details`）；`orders` 維持隨機永久 token 不變。
- `share_token_for_code(code)`（SECURITY DEFINER）＝ `extensions.hmac(code::bytea, (SELECT value FROM app_secrets WHERE key='share_token_v1')::bytea, 'sha256')` 取 hex 前 16 bytes 組 UUID 8-4-4-4-12。同 code 同 token、決定性。⚠️ hmac 在 `extensions` schema，search_path=public 下必須 `extensions.hmac` 否則 42883。
- `app_secrets` 表：RLS 僅 admin（policy `app_secrets_admin_all`），seed `share_token_v1 = encode(gen_random_bytes(32),'hex')`。
...
| `correct_sales_note(p_sales_note_id, p_items_to_remove UUID[] DEFAULT '{}', p_items_to_add JSONB DEFAULT '[]', p_new_items JSONB DEFAULT '[]', p_created_by, p_price_updates JSONB DEFAULT '[]')` | **銷貨單修正（2026-09-11, migration `20260911000007_correct_sales_note.sql`＋`20260911000008_correct_sales_note_add_price_updates.sql`，20260911000009 已 DROP 舊 5 參數 overload，現為**單一 6 參數**簽名）**：僅 admin；在**不失效 QR code / access_token**（欄位不動）的前提下修正已出貨銷貨單品項，RETURNS JSONB。守門：`status <> 'received'`、無會計分錄（two-path）、無 `rep_commission_payouts`、無未 reversed 的 `consignment_sales`。**Phase 1 移除**（sales_note_item ids）：先 DELETE 舊 `sales_shipment` movement（釋放 `(sales_note_id, order_item_id)` unique 索引）→ 新增 reversal movement（own 倉 `sales_note_deletion`；`store_consignment`→`consignment_shipment_reversal`、`supplier_consignment`→`consignment_sale_reversal`，owner 同源）回勾庫存 → 回退 `order_items.shipped_quantity/status` → 品項退回 `shipping_pool`（`UNIQUE(order_item_id)` 故 SELECT/UPDATE 覆寫或 INSERT，`trg_shipping_pool_auto_sort_order` 自動派號）→ DELETE sales_note_item。**Phase 2 追加**（`[{order_item_id, quantity}]`，出貨池或同店家訂單未出貨量；守門 `.eq store_id` 跨店擋下）：剩餘量 `quantity - shipped_quantity >= quantity` → 新增 sales_note_item（`sort_order=MAX+1`）→ 依 `order_items.inventory_source_type` 扣庫存（store_consignment 走 `allocate_inventory`、其餘 `sales_shipment`）→ 回填 shipped_quantity/status → 出貨池移除該 order_item。**Phase 3 完全新品**（`[{product_id, variant_id?, quantity, unit_price}]`）：先建 `orders`（`source_type='admin_proxy'`、`status='shipped'`、`consignment_mode=false`、店名「系統自動建立（銷貨單修正）」）＋ order_item（status='shipped'、`shipped_quantity=quantity`、`unit_price` fallback `COALESCE(p.unified_wholesale_price, p.unified_retail_price)`）→ sales_note_item（`inventory_source_type='self'`）→ 扣損 `sales_shipment`。**Phase 4 價格更新（2026-09-11）**（`p_price_updates=[{order_item_id, new_unit_price}]`）：直接改 `order_items.unit_price`（**銷貨單無自價格**，所有讀取皆 `order_item.unit_price`，單一資料源自動反映）——守門：`new_unit_price>=0`、order_item 須屬此銷貨單；其他**已收款**銷貨單（`.neq` 自己＋`sn.payment_status='paid'`）引用同一 order_item 時 RAISE 擋下；其他未收款引用收集到 `price_updates[].other_affected_sales_notes`（前端警示展示，不擋）。回傳 progress 於 `price_updates[]`（`order_item_id/product_id/variant_id/old_unit_price/new_unit_price/order_code/other_affected_sales_notes`）。**Phase 5**：touch `updated_at`、受影響訂單全數出貨收斂 `shipped`、回傳 items 快照 ＋ `price_updates`。`REVOKE ALL...GRANT EXECUTE TO authenticated` |
...
| `reverse_consignment_shipment(p_consignment_order_id, p_created_by, p_note)` | **出貨回滾（v1.5 新）**：整單回滾店家方向出貨（見下方 v1.5 說明），RETURNS JSONB `{consignment_order_id, source_order_id, reversed_items, reversed_quantity}`；成功時扣回出貨、品項放回 shipping_pool（**覆寫 quantity 非累加**，v1.5.1）、寄賣單回 draft、來源 order_items 回 waiting/來源訂單全回滾時降 processing。**2026-09-11 bug 修復（`20260804000002`）**：移除會誤配商品的 lookup（篩 `oi.source_order_id IS NOT NULL`＋`LIMIT 1` 卻無 product/variant 條件，A+B 拆單時可能對到錯品項），保留以 product/variant＋`source_order_id` 精確比對的既存路徑 |
...
- **變體批次更新（2026-09-10，migration `20260910000001_batch_upsert_product_options.sql`）**：`batch_upsert_product_options(p_product_id, p_groups, p_variants, p_variant_options, p_model_relations)` —— SECURITY DEFINER（僅 admin），把 `VariantBatchCreator` 的 destroy-then-rebuild 包在**單一 transaction**（任一環節失敗 ROLLBACK）。payload 以「客戶端臨時 id（ref）」串接後由 RPC 解析實際 UUID：`p_groups=[{ref,name,values:[{ref,label,value,hex_code}]}]`、`p_variants=[{sku,name,barcode,wholesale_price,retail_price,sort_order}]`（依 sku 去重、onConflict `sku` upsert）、`p_variant_options=[{sku,group_ref,value_ref}]`、`p_model_relations=[{sku,model_id?|group_id?,sort_order}]`（relation_type 固定 `include`，先刪該產品變體所有 include 再重建）；最後 `PERFORM sync_storefront_items(p_product_id)`。回 `{ok:false, reason}` 表示權限不足或產品不存在。**⚠️ jsonb 陣列參數一律直接傳 JS 陣列、勿 `JSON.stringify()`，否則 PostgREST 解成 string scalar，`jsonb_array_elements(scalar)` 拋 22023 且整筆 ROLLBACK。**防呆 migration `20260910000002_batch_upsert_product_options_harden.sql` 在每個 `jsonb_array_elements` 前以 `jsonb_typeof` 檢查、非陣列視為空陣列（簽名不變）。
- **變體選項原子更新（2026-09-12，migration `20260912000000_upsert_variant_options.sql`）**：`upsert_variant_options(p_variant_id, p_items)` —— SECURITY DEFINER（僅 admin），將 `VariantEditDialog` 的 `manageVariantOptions`（5~8 次前端 HTTP，非交易）改為**單一 transaction**：依 `(label OR value)` 對齊選項值（不存在 INSERT、存在且 hex_code 為 NULL 時補色庫 hex_code）→ DELETE+INSERT `product_variant_options` → orphan 清理（該產品所有群組中無任何變體使用的 `product_option_values`）。`p_items=[{group_id,label}]`，直接傳 JS 陣列、不 `JSON.stringify`。
